...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    for chunk in chunks:
        if platform == Platform.DISCORD:
            ...
            result = await _send_qqbot(pconfig.extra, chat_id, chunk)
        else:
...
async def _send_qqbot(extra, chat_id, message):
    """Send via QQ Bot using the adapter's WebSocket send pipeline."""
    try:
        from gateway.platforms.qqbot import QQBotAdapter, check_qqbot_requirements
        if not check_qqbot_requirements():
            return {"error": "QQBot requirements not met. Need websockets + QQBOT_APP_ID/TOKEN/SECRET."}
    except ImportError:
    ...
    try:
        ...
        connected = await adapter.connect()
        if not connected:
            return _error(f"QQBot: failed to connect - {adapter.fatal_error_message or 'unknown error'}")
        try:
            result = await adapter.send(chat_id, message)
            if not result.success:
